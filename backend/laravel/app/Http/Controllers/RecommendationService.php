...
class RecommendationService
{
    ...
    /**
     * Automated actuation decision -- a direct port of Algorithm 1, step 15
     * (manuscript Table 2). Given the fused pest class and the LSTM plant-health
     * status it returns the single control action the AIoT system should take:
     *
     *   pest detected & class = Hispa  -> ultrasonic repeller + UV lamp
     *   pest detected (other)          -> selective insecticide spraying (advisory)
     *   status = High Stress / VPD>=1.5 -> irrigation notification
     *   otherwise                      -> continue monitoring
     *
     * @param  iterable      $sensorData   latest sensor rows (temperature, humidity)
     * @param  iterable      $cameraData   latest CV rows (penyakit)
     * @param  string|null   $healthStatus predicted_health from the LSTM forecast
     */
    public function getActuation($sensorData, $cameraData, ?string $healthStatus = null): array
    {
        $HISPA   = 'Hama penggerek daun (Hispa)';
        $HEALTHY = 'Normal / Sehat';

        // ---- pest class from the latest CV / fused detection ----
        $penyakit = null;
        foreach ($cameraData as $row) {
            $penyakit = is_array($row) ? ($row['penyakit'] ?? null) : ($row->penyakit ?? null);
            break;
        }
        $pestDetected = $penyakit !== null && $penyakit !== '' && $penyakit !== $HEALTHY;

        // ---- VPD from the latest sensor row (Tetens/Magnus) ----
        $vpd = null;
        foreach ($sensorData as $row) {
            $t  = (float) (is_array($row) ? ($row['temperature'] ?? 0) : ($row->temperature ?? 0));
            $rh = (float) (is_array($row) ? ($row['humidity'] ?? 0) : ($row->humidity ?? 0));
            if ($t > 0) {
        ...
        if ($pestDetected) {
            if ($penyakit === $HISPA) {
                return [
                    'action'    => 'activate_ultrasonic_repeller_and_uv_lamp',
                    'actuators' => ['ultrasonic_repeller', 'uv_lamp'],
                    'type'      => 'danger',
                    'icon'      => 'bx bx-broadcast',
                    'title'     => 'Aktuasi: Ultrasonik + Lampu UV',
                    'message'   => "Terdeteksi {$penyakit}. Sistem mengaktifkan pengusir ultrasonik dan lampu UV secara otomatis.",
                    'pest'      => $penyakit,
                    'reason'    => 'pest detected (Hispa)',
                ];
            }
            return [
                'action'    => 'recommend_selective_insecticide_spraying',
                'actuators' => [],
                'type'      => 'warning',
                'icon'      => 'bx bx-spray-can',
                'title'     => 'Rekomendasi: Penyemprotan Insektisida Selektif',
                'message'   => "Terdeteksi {$penyakit}. Lakukan penyemprotan insektisida selektif sesuai anjuran agronomi.",
                'pest'      => $penyakit,
                'reason'    => 'pest detected',
            ];
        }

        if ($healthStatus === 'High Stress' || ($vpd !== null && $vpd >= 1.5)) {
            return [
                'action'    => 'irrigation_notification',
                'actuators' => ['irrigation_valve'],
                'type'      => 'danger',
                'icon'      => 'bx bx-water',
                'title'     => 'Notifikasi: Irigasi',
                'message'   => $healthStatus === 'High Stress'
                    ? 'Status tanaman: High Stress. Sistem mengirim notifikasi irigasi.'
                    : sprintf('VPD tinggi (%.2f kPa >= 1.5). Sistem mengirim notifikasi irigasi.', $vpd),
                'pest'      => null,
                'reason'    => $healthStatus === 'High Stress' ? 'plant health = High Stress' : 'VPD >= 1.5 kPa',
            ];
        }
        ...
        return [
            ...
            'title'     => 'Pemantauan Berlanjut',
            'message'   => 'Tidak ada hama terdeteksi dan kondisi tanaman aman. Sistem melanjutkan pemantauan.',
            'pest'      => null,
            'reason'    => 'no pest, health OK',
        ];
    }
}
