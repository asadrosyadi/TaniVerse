    ...
    <div class="col-md-12 col-lg-12 order-5">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0"><i class="bx bx-history me-2"></i>{{ __('label.riwayat_yolo') }}</h5>
                <div class="d-flex">
                    <a href="{{ route('export.data.yolo.user', $iot_idUser->iot_id) }}" class="btn btn-sm btn-outline-primary d-flex align-items-center me-2">
                    ...
                    <table class="table table-hover table-sm">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>{{ __('label.diseases') }}</th>
                                <th>{{ __('label.probability') }} (%)</th>
                        ...
                        <tbody id="sensorDataBody">
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-icon btn-outline-primary" onclick="openImage('{{ asset('img/hama_padi/' . $item['image']) }}')">
                ...
                <nav aria-label="Page navigation" class="mt-3">
                    <ul class="pagination justify-content-center pagination-sm">
                        ...
                        <li class="page-item {{ $currentPage == 1 ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ url('/yolo?page=' . ($currentPage - 1)) }}" aria-label="Previous">
                        ...
                        @for ($i = $startPage; $i <= $endPage; $i++)
                            <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                                <a class="page-link" href="{{ url('/yolo?page=' . $i) }}">{{ $i }}</a>
                            </li>
                        @endfor

                        {{-- Next Button --}}
                        <li class="page-item {{ $currentPage == $totalPages ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ url('/yolo?page=' . ($currentPage + 1)) }}" aria-label="Next">
...
<script>
    $(document).ready(function() {
        $('#refreshButton').on('click', function() {
            ...
            window.location.href = "/yolo?page=" + currentPage;
        });
