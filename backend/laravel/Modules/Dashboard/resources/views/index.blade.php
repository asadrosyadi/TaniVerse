...
<div class="row justify-content-center">
                ...
                <div class="card detection-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <div class="detection-details">
                            ...
                            {{-- Hasil model fusi CNN-LSTM (Eq. 1) --}}
                            <div class="alert alert-primary p-2 small mb-3" id="hybridFusion"
                                 @unless(isset($hybridInference) && $hybridInference) style="display:none" @endunless>
                                @isset($hybridInference)
                                    @if($hybridInference)
                                        <strong><i class="bx bx-git-merge"></i> Hybrid CNN&ndash;LSTM</strong>
                                        (w1={{ $hybridInference->fusion_w1 ?? '-' }}, w2={{ $hybridInference->fusion_w2 ?? '-' }})<br>
                                        Hama: {{ $hybridInference->pest_label ?? '-' }} &middot;
                                        Kesehatan: {{ $hybridInference->health_status ?? '-' }} &middot;
                                        VPD: {{ !is_null($hybridInference->vpd_kpa) ? number_format($hybridInference->vpd_kpa, 3).' kPa' : '-' }}
                                    @endif
                                @endisset
                            </div>
    ...
    <!-- Automated Actuation (Algorithm 1 - Hybrid CNN-LSTM) -->
    <div class="col-lg-4 order-3 order-lg-2 mb-4 mt-4">
        <div class="card h-100">
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0 text-white"><i class="bx bx-cog me-2"></i> Aksi Aktuasi Otomatis</h5>
                @isset($hybridInference)
                    @if($hybridInference)
                        <span class="badge bg-white text-primary">CNN&ndash;LSTM Fusion</span>
                    @endif
                @endisset
            </div>
            <div class="card-body" id="actuation">
                @isset($actuation)
                    <div class="alert alert-{{ $actuation['type'] ?? 'info' }} d-flex align-items-start p-3 mb-2">
                        <i class="{{ $actuation['icon'] ?? 'bx bx-cog' }} fs-4 me-3"></i>
                        <div>
                            <strong class="d-block">{{ $actuation['title'] ?? 'Aksi' }}</strong>
                            <p class="mb-1 small">{{ $actuation['message'] ?? '' }}</p>
                            <code class="small">{{ $actuation['action'] ?? '' }}</code>
                            @if(!empty($actuation['actuators']))
                                <div class="mt-1">
                                    @foreach($actuation['actuators'] as $act)
                ...
                @isset($hybridInference)
                    @if($hybridInference)
                        <ul class="list-unstyled small mb-0">
                            <li><strong>Hama:</strong> {{ $hybridInference->pest_label ?? '-' }}
                                @if(!is_null($hybridInference->pest_confidence))
                                    ({{ number_format($hybridInference->pest_confidence * 100, 1) }}%)
                                @endif
                            </li>
                            <li><strong>Kesehatan:</strong> {{ $hybridInference->health_status ?? '-' }}
                                @if(!is_null($hybridInference->health_confidence))
                                    ({{ number_format($hybridInference->health_confidence * 100, 1) }}%)
                                @endif
                            </li>
                            <li><strong>VPD:</strong>
                                {{ !is_null($hybridInference->vpd_kpa) ? number_format($hybridInference->vpd_kpa, 3) . ' kPa' : '-' }}
                            </li>
                            <li><strong>Bobot fusi:</strong>
                                w1={{ $hybridInference->fusion_w1 ?? '-' }}, w2={{ $hybridInference->fusion_w2 ?? '-' }}
                            </li>
                            <li><strong>Prob. hama (softmax):</strong> {{ $hybridInference->pest_class ?? '-' }}</li>
                            <li class="text-muted">via {{ $hybridInference->transmitted_via ?? 'rest' }}
                                &middot; {{ optional($hybridInference->created_at)->diffForHumans() }}</li>
                        </ul>
                    @else
                        <p class="text-muted small mb-0" id="hybridEmpty">
                            Belum ada inferensi model fusi CNN&ndash;LSTM (edge node belum mengirim ke
                            <code>POST /api/inference</code>).
                        </p>
                    @endif
                @endisset
            </div>
...
<script>
    ...
    function fetchRecommendations(page = 1, days = 3) {
        $.get('/getRecommendations', { page: page, days: days }, function(response) {
            ...
            if (response.actuation) {
                var a = response.actuation;
                var hi = response.hybridInference;
                var actuatorBadges = (a.actuators || []).map(function(x) {
                    return `<span class="badge bg-primary">${x}</span>`;
                }).join(' ');
                var pct = function(v) { return (v === null || v === undefined) ? '' : ' (' + (v * 100).toFixed(1) + '%)'; };
                var hiHtml = '';
                if (hi) {
                    hiHtml = `
                        <ul class="list-unstyled small mb-0">
                            <li><strong>Hama:</strong> ${hi.pest_label || '-'}${pct(hi.pest_confidence)}</li>
                            <li><strong>Kesehatan:</strong> ${hi.health_status || '-'}${pct(hi.health_confidence)}</li>
                            <li><strong>VPD:</strong> ${hi.vpd_kpa != null ? Number(hi.vpd_kpa).toFixed(3) + ' kPa' : '-'}</li>
                            <li><strong>Bobot fusi:</strong> w1=${hi.fusion_w1 != null ? hi.fusion_w1 : '-'}, w2=${hi.fusion_w2 != null ? hi.fusion_w2 : '-'}</li>
                            <li class="text-muted">via ${hi.transmitted_via || 'rest'}</li>
                        </ul>`;
                }
                $('#actuation').html(`
                    <div class="alert alert-${a.type || 'info'} d-flex align-items-start p-3 mb-2">
                        <i class="${a.icon || 'bx bx-cog'} fs-4 me-3"></i>
                        <div>
                            <strong class="d-block">${a.title || 'Aksi'}</strong>
                            <p class="mb-1 small">${a.message || ''}</p>
                            <code class="small">${a.action || ''}</code>
                            ${actuatorBadges ? '<div class="mt-1">' + actuatorBadges + '</div>' : ''}
                        </div>
                    </div>` + hiHtml);

                // Ringkasan model fusi di kartu deteksi (Figure 11c)
                if (hi) {
                    var vpdTxt = (hi.vpd_kpa != null) ? Number(hi.vpd_kpa).toFixed(3) + ' kPa' : '-';
                    $('#hybridFusion').html(
                        `<strong><i class="bx bx-git-merge"></i> Hybrid CNN&ndash;LSTM</strong> ` +
                        `(w1=${hi.fusion_w1 != null ? hi.fusion_w1 : '-'}, w2=${hi.fusion_w2 != null ? hi.fusion_w2 : '-'})<br>` +
                        `Hama: ${hi.pest_label || '-'} &middot; Kesehatan: ${hi.health_status || '-'} &middot; VPD: ${vpdTxt}`
                    ).show();
                }
            }
