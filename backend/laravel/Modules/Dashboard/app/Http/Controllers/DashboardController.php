...
class DashboardController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        // Algorithm 1, step 15 -- automated actuation decision, plus the latest
        // unified hybrid CNN-LSTM inference (if the edge node has POSTed one).
        $forecastData_first = $forecastData_first ?? null;
        $predictedHealth = is_array($forecastData_first) ? ($forecastData_first['predicted_health'] ?? null) : null;
        $actuation = $recommendationService->getActuation($sensorData, $sensorData2, $predictedHealth);
        $hybridInference = HybridInference::where('iot_id', $iot_id)->orderByDesc('id')->first();
    ...
    public function getDataPrediksi()
    {
        ...
        // Algorithm 1, step 15 -- actuation decision (uses the 1-day predicted health)
        $forecast1 = $this->getForecastData($iot_id, 1);
        if (is_string($forecast1)) {
            $forecast1 = json_decode($forecast1, true);
        }
        $predictedHealth = null;
        if (isset($forecast1['forecast']) && is_array($forecast1['forecast'])) {
            $first = collect($forecast1['forecast'])->sortBy('timestamp')->first();
            $predictedHealth = $first['predicted_health'] ?? null;
        }
        $actuation = $recommendationService->getActuation($sensorData, $sensorData2, $predictedHealth);
        $hybridInference = HybridInference::where('iot_id', $iot_id)->orderByDesc('id')->first();
